...
cluster = tf.train.ClusterSpec({"ps": ["mist37-umh.cs.umn.edu:2222"],
                                "worker": ["mist37-umh.cs.umn.edu:2222"]})
...
if job_name == "ps":
    ...
    with tf.device("/job:ps/task:0"):
        ...
        W_conv1 = weight_variable([11, 11, 3, 96]) # I am not sure about 96 ... As I understand the 96 is the number of Kernels
        ...
        W_fc1 = weight_variable([8*8*64, 4096])## not sure about that 12*12*256
    ...
    with tf.device(tf.train.replica_device_setter(worker_device="/job:worker/task:%d" % task_number, cluster=cluster)):
        ...
        '''
        conv_layer1 = tf.nn.relu(conv2d(picture, W_conv1,stride = [1, 4, 4, 1]) + b_conv1) ## chan you just add? don't you have to do np.add?
        radius = 2; alpha = 2e-05; beta = 0.75; bias = 1.0
        normalized_layer1 = tf.nn.local_response_normalization(conv_layer1, depth_radius=radius, alpha=alpha, beta=beta, bias=bias)
        layer1 = max_pool(normalized_layer1, k = [1, 3, 3, 1], pad='VALID')
        ...
        normalized_layer2 = tf.nn.local_response_normalization(conv_layer2, depth_radius=radius, alpha=alpha, beta=beta, bias=bias)
        ...
        layer5 = max_pool(tf.nn.relu(conv2d(layer4, W_conv5) + b_conv5),k = [1, 3, 3, 1],pad='VALID')


        layer5_flat =  tf.reshape(layer5, [-1, int(prod(layer5.get_shape()[1:]))])
        fc1 = tf.nn.relu_layer(layer5_flat,W_fc1,b_fc1)

        fc2 = tf.nn.relu_layer(fc1, W_fc2, b_fc2)

        fc3 = tf.nn.xw_plus_b(fc2, W_fc3, b_fc3)
